...
def parse():
    ...
    parser.add_argument('--mix_alpha', default=1, type=float)
...
if __name__ == '__main__':
    ...
    data_path = os.path.join('../data-label/', args.data)
    ...
    if args.data == 'rcv1':
        label_des = torch.load(os.path.join(data_path, 'label_description.pt'))
    ...
    if args.model == 'prompt':
        if os.path.exists(os.path.join(data_path, args.model + '_prefix')):
            dataset = datasets.load_from_disk(os.path.join(data_path, args.model + '_prefix'))
        else:
            dataset = datasets.load_dataset('json',
                                            data_files={'train': '../data-label/{}/{}_train.json'.format(args.data, args.data),
                                                        'dev': '../data-label/{}/{}_dev.json'.format(args.data, args.data),
                                                        'test': '../data-label/{}/{}_test.json'.format(args.data, args.data), })
            prefix = []
            ...
            def data_map_function(batch, tokenizer):
                ...
                for l, t in zip(batch['label'], batch['token']):
                    ...
                    all_label_flat = []
                    for i in range(max_depth):
                        idx = tokenizer.vocab_size + num_class + i
                        temp_label = ''
                        for label in l:
                            if depth_dict[label] == i:
                                temp_label += ' ' + label_dict[label]
                        temp_label = tokenizer(temp_label, truncation=True)['input_ids']
                        temp_label = [idx] + temp_label[1: -1]
                        all_label_flat += temp_label
                    all_label_flat = [tokenizer.cls_token_id] + all_label_flat + [tokenizer.sep_token_id]
                    new_batch['labels'].append([[-100 for _ in range(num_class)] for _ in range(max_depth)])  # depth * numclass
                    ...
                    if len(tokens['input_ids'][1:]) + len(prefix) > 512:
                        new_batch['input_ids'].append(prefix + tokens['input_ids'][1:][:512 - len(prefix) - 1] + [tokenizer.sep_token_id])
                    else:
                        new_batch['input_ids'].append(prefix + tokens['input_ids'][1:][:512 - len(prefix)])
                    new_batch['input_ids'][-1].extend([tokenizer.pad_token_id] * (512 - len(new_batch['input_ids'][-1])))
                    new_batch['attention_mask'].append([1] * len(prefix) + tokens['attention_mask'][:-1][:512 - len(prefix)])
                    new_batch['attention_mask'][-1].extend([0] * (512 - len(new_batch['attention_mask'][-1])))
                    new_batch['token_type_ids'].append([0] * 512)
                    new_batch['all_label_flat'].append(all_label_flat)
                    new_batch['all_label_flat'][-1].extend([tokenizer.pad_token_id] * (512 - len(new_batch['all_label_flat'][-1])))
                return new_batch


            dataset = dataset.map(lambda x: data_map_function(x, tokenizer), batched=True)
            dataset.save_to_disk(os.path.join(data_path, args.model + '_prefix'))
        dataset['train'].set_format('torch', columns=['attention_mask', 'input_ids', 'labels', 'all_label_flat'])
        dataset['dev'].set_format('torch', columns=['attention_mask', 'input_ids', 'labels', 'all_label_flat'])
        dataset['test'].set_format('torch', columns=['attention_mask', 'input_ids', 'labels', 'all_label_flat'])

        from models.prompt import Prompt

    else:
        raise NotImplementedError
    if args.low_res:   # default false
    ...
    model = Prompt.from_pretrained(args.arch, num_labels=len(label_dict), path_list=path_list, layer=args.layer,
                                   data_path=data_path, depth2label=depth2label, mix_alpha=args.mix_alpha, 
                                   warm_up=args.warm_up, threshold=args.threshold, alpha=args.alpha)
    ...
    dev = DataLoader(dataset['dev'], batch_size=32, shuffle=False)
    model.to('cuda')
    ...
    def test_function(extra):
        ...
        micro_f1 = scores['micro_f1']
        print('macro', macro_f1, 'micro', micro_f1)
        print("%.2f" % (micro_f1 * 100))
        print("%.2f" % (macro_f1 * 100))
